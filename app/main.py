...
def _compute_sector_proportionality_model(
    request: Request,
    sector: str,
    tickers: list[str],
    *,
    ...
    def _add_cached_pairs_for_ticker(ticker: str) -> None:
        nonlocal pairs
        surprise_data = get_ticker_surprise(db, ticker, None)
        ...
        for filing_date, surprise_val in surprise_data.items():
            ...
            reaction_data = get_ticker_reaction(db, ticker, filing_date=filing_date_norm)
            if reaction_data is None:
                continue
            series = reaction_data.get(filing_date_norm) or {}
            if not series:
                continue
    ...
    for ticker in tickers:
        _add_cached_pairs_for_ticker(ticker)
        if len(pairs) >= max_pairs:
            break
...
async def fetch_proportionate_for_ticker(
    ...
    if filings_date is None:
        raise HTTPException(
            status_code=400,
            detail="filings_date is required for /proportionate (YYYY-MM-DD)",
        )
    ...
    if proportionality_data is None:
        ...
        db = cast(SQLiteDatabase, request.app.state.database)
        mean_surp, sd_surp, alpha, beta = _compute_sector_proportionality_model(
    ...
    surprise_value = float(surprise_data["surprise"][filings_date])
