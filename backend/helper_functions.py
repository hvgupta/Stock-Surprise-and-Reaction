...
#     sbac: AsyncClient, ticker: str, trailing_eps: float, forward_eps: float, date: str
# ) -> float:
#     date = normalize_date_str(date)
#     surprise = calc_surprise_of_ticker(trailing_eps, forward_eps)
#     await upsert_eps_data_of_ticker(sbac, ticker, date, trailing_eps, forward_eps)
#     await upsert_surprise_data(sbac, ticker, date, surprise)
#     return surprise

async def get_reaction_for_date(
    sbac: AsyncClient,
    ticker: str,
    cur_filing_date: str,
